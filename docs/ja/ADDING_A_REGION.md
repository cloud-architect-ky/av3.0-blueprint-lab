...
| `ml.g5.24xlarge` | `L-F087CCFC` | 2 | 2 | M4/M5/M6（24 GB ティア） |
...
| **`ml.g6.24xlarge`** | `L-8ACE1754` | 2 | **0** ⚠ | M4〜M9 のドキュメント上のデフォルト |
...
ソウルで実行できます: M2/M3 には `ml.g5.12xlarge`、M4/M5/M6 には `ml.g5.24xlarge` を使い（g6 の
デフォルトと同じ GPU あたり約 22.5 GB のティアで、約 22% 高価）、品質重視の選択肢として
`ml.p4d.24xlarge` を使います。ドキュメント上のデフォルトを使いたい場合にのみ `g6` を申請して
ください — なお、いずれにせよそこでの価格は us-west-2 より約 23% 高いことに注意してください。
...
もデプロイの最後にこれを実行します。このアカウントで参加者 10 人として実測した結果: 現在の構成では
**どちらの**リージョンも合格しません — `ml.g6.24xlarge` は 4 つのモジュールが要求しますが、同時実行は
us-west-2 で 2、ap-northeast-2 で 0 しか許可されず、`ml.g5.12xlarge` /
`ml.g5.xlarge` は 5 です。これを前提に受講者数を計画するか、クォータを引き上げてください:
