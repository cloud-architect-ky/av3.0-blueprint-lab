...
ap-northeast-2 は 5 対 **0**）、そしてラボが実際に検証したファミリーです — M9 の合格実行
（minADE 0.3779、`Status: PASS`）は g5、つまり A10G でのものでした。本ガイドの以前の版は
`ml.g6.24xlarge` を「今サイクルで検証」と記載していましたが、これは A10G の結果の誤った帰属でした。
