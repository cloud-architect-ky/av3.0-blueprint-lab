...
- **아무 GPU 인스턴스에서나 되지 않습니다** — 두 모델 모두 한 GPU에 올리려면 ~65 GB가
  필요합니다. 24 GB 카드에서는 **GPU 2장 이상**이 있어야 샤딩으로 실행되며, 1장이면
  노트북이 `RuntimeError`로 멈추고 멀티 GPU 인스턴스를 고르라고 안내합니다
  (M5 cell 2 / M6 cell 2). p5.48xlarge(H100×8)와 p 계열에서 검증되었습니다.
