...
워크스페이스는 **200 GB** EBS 볼륨으로 생성되며, 이는 모든 모듈을 커버합니다.
**Instance Options** 패널은 볼륨의 *실제* 크기와 그 모듈의 *권장* 용량을 함께 보여줍니다 —
서로 다른 숫자이고 패널이 그렇게 구분해 표시합니다. 노트북이 실제로 디스크가 부족하면 거기서
**+50 GB / +200 GB**를 추가할 수 있습니다. 볼륨은 늘릴 수는 있지만 **줄일 수는 없습니다**.
